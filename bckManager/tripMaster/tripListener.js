...
async function runTripManager() {
    if (isTripManagerRunning) {
        console.log(getCurrentTimeIST(), 'TripMaster is already running');
        return;
    }
    ...
    try {
        ...
        const tripManager = spawn('node', [path.join(__dirname, 'tripMaster.js')]);
        ...
        tripManager.stderr.on('data', (data) => {
            console.error(getCurrentTimeIST(), `TripMaster stderr: ${data}`);
        });

        // Wait for tripManager to complete
        await new Promise((resolve, reject) => {
            tripManager.on('close', (code) => {
                console.log(getCurrentTimeIST(), `TripMaster exited with code ${code}\n\n\n`);
                console.log("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
            ...
            tripManager.on('error', (error) => {
                console.error(getCurrentTimeIST(), 'TripMaster error:', error);
                isTripManagerRunning = false;
                reject(error);
            });
        });
        ...
        console.error(getCurrentTimeIST(), 'Error running TripMaster:', error);
        isTripManagerRunning = false;
    }
}
...
async function init() {
    try {
        ...
        // Run TripManager immediately and then schedule recurring execution
        await runTripManager();
        setInterval(async () => {
            await runTripManager();
        }, tripManagerInterval * 60 * 1000); // Run every tripManagerInterval minutes
